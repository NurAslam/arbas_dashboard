...
with tab4:
    ...
    status_region = pd.crosstab(df_filtered['daerah'], df_filtered['status'])

    fig_status_region = px.bar(
        status_region.reset_index(),
        x='daerah',
        y=status_region.columns,
        title="Distribusi Status per Daerah",
        barmode='stack',
        labels=dict(value="Jumlah", daerah="Daerah", variable="Status", color="Status")
    )
    st.plotly_chart(fig_status_region, use_container_width=True)

with tab5:
    st.subheader("Korelasi dan Insight")

    st.info("""
    💡 **Analisis ini bertujuan memahami pola pengambilan data baru oleh admin**
    untuk diserahkan ke tim sales. Fokus pada karakteristik berdasarkan wilayah,
